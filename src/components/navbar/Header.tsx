...
const Header = async ({ slug }: { slug: string }) => {
  ...
  const pages = await prisma.page.findMany({
    where: {
      schoolId: school.id,
      parentId: null, // Only fetch top-level pages
    },
    include: {
      children: {
        include: {
          children: {
            include: {
              children: true, // You can go deeper if needed
            },
          },
        },
      },
  ...
  return (
    <header className="p-4 lg:p-0  gradient relative">
      <Container className="flex h-full items-center flex-col">
        <div className="flex flex-col sm:flex-row items-center h-full w-full">
          {/* logo */}
          <div className="relative w-[80px] sm:w-[200px] aspect-square flex items-center justify-center mb-2 lg:mb-0">
            {school.logoUrl ? (
              <Image
                fill
                src={school.logoUrl}
          ...
          <div className="flex flex-col text-white w-full">
            ...
            {/* <h2
              className={`${trio.className} text-xl text-center sm:text-2xl lg:text-3xl xl:text-5xl mb-4 lg:mb-0`}
            >
        ...
        {/* main navbar */}
        <Navbar pages={pages} school={slug} />
      </Container>
    </header>
  );
};
